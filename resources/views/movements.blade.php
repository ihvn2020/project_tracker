...
@section('content')
    ...
    <div class = "row" style="width:98%; margin:auto;" id="printable" data-logo="{{$site_settings->logo}}">
                ...
                <tr>
                    ...
                    <td>
                        @if(substr($mv->file, -3)=="jpg" || substr($mv->file, -3)=="png" || substr($mv->file, -3)=="peg")
                          <img src="/uploads/{{$mv->id}}/{{$mv->file}}" height="80" width="auto"/>
                        @elseif(substr($mv->file, -3)=="pdf")
                          <a href="#fileModal" id="viewfile" onclick="viewFile()" class="btn-floating btn-small waves-effect waves-light btn modal-trigger tooltipped" data-position="top" data-tooltip="View / Edit Item" data-src="/uploads/{{$mv->id}}/{{$mv->file}}" data-filename="{{$mv->item_name}}"><i class="material-icons">remove_red_eye</i></a>
                        @elseif(substr($mv->file, -3)=="doc" || substr($mv->file, -3)=="ocx" || substr($mv->file, -3)=="ptx" || substr($mv->file, -3)=="ppt" || substr($mv->file, -3)=="xls")
  
                        <a href="/uploads/{{$mv->id}}/{{$mv->file}}">Open File</a>
                          <!--<a href="#fileModal" id="viewfile" onclick="viewFile()" class="btn-floating btn-small waves-effect waves-light btn modal-trigger tooltipped" data-position="top" data-tooltip="View / Edit Item" data-src="https://view.officeapps.live.com/op/view.aspx?src=http://localhost/uploads/{{$mv->id}}/{{urlencode($mv->file)}}" data-filename="{{$mv->item_name}}"><i class="material-icons">remove_red_eye</i></a>
  
                          <a href="#fileModal" id="viewfile" onclick="viewFile()" class="btn-floating btn-small waves-effect waves-light btn modal-trigger tooltipped" data-position="top" data-tooltip="View / Edit Item" data-src="http://docs.google.com/gview?url=http://localhost/uploads/{{$mv->id}}/{{urlencode($mv->file)}}" data-filename="{{$mv->item_name}}"><i class="material-icons">remove_red_eye</i></a>-->
                          
                        @elseif($mv->file=="Multiple Files")
                        @php
                            $files = scandir(public_path() . '/uploads/'.$mv->inventories_id.'/');
                            foreach ($files as $key => $file) {
                                echo '<a href="/uploads/'.$mv->id.'/'.$file.'" target="_blank">'.$file.'</a><br>';
                            }
                        @endphp
                    ...
                    <td>                    
                        <div class="fixed-action-btn horizontal direction-top direction-left click-to-toggle sales_action" style="position: relative !important; float: text-align: center; display: inline-block; bottom: 0px !important; padding: 0px !important">
                                ...
                                <ul style="top: 0px !important">
                                    ...
                                    <li>
                                            <a href="/item/{{$mv->id}}" class="btn-floating btn-small waves-effect blue waves-light tooltipped" data-position="top" data-tooltip="View Item History"><i class="material-icons">remove_red_eye</i></a>         
                                    </li>
        ...
        <div class="col m6 offset-m3">{{$movements->links()}}</div>
        @else
